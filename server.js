...
// 1. MÃ HÓA CHUỖI RUN (chuỗi liên tiếp cùng cửa)
...
// 2. THỐNG KÊ TẦN SUẤT TỔNG ĐIỂM
function thongKeTong(items) {
  const dem = {};
  items.forEach(x => {
    const tong = x.tong || (x.xuc_xac_1 + x.xuc_xac_2 + x.xuc_xac_3);
...
(async () => {
  try {
    console.log('=== THUẬT TOÁN PHÂN TÍCH CẦU TÀI XỈU SUNWIN ===\n');
    const items = await layDuLieu();
    const seq = taoChuoi(items);
    const phienHienTai = items[items.length - 1].phien;

    console.log(`Tổng dữ liệu: ${items.length} phiên`);
    console.log(`Phiên hiện tại: #${phienHienTai}`);
    console.log(`Phiên dự đoán: #${phienHienTai + 1}\n`);

    // 20 phiên gần nhất
    console.log('--- 20 PHIÊN GẦN NHẤT ---');
    console.log(seq.slice(-20).join(' '));
    console.log('');

    // thống kê tổng
    const tai = seq.filter(x => x === 'TAI').length;
    const xiu = seq.length - tai;
    console.log(`Tổng: TÀI ${tai} (${(tai/seq.length*100).toFixed(1)}%) | XỈU ${xiu} (${(xiu/seq.length*100).toFixed(1)}%)\n`);

    // thống kê tổng điểm
    const tongDiem = thongKeTong(items.slice(-500));
    console.log('--- TẦN SUẤT TỔNG ĐIỂM 500 PHIÊN ---');
    for (let i = 3; i <= 18; i++) {
      const c = tongDiem[i] || 0;
      const pct = (c / 500 * 100).toFixed(1);
      const bar = '█'.repeat(Math.round(c / 5));
      console.log(`  ${i.toString().padStart(2)}: ${c.toString().padStart(3)} (${pct}%) ${bar}`);
    }
    console.log('');

    // phân tích chi tiết
    const runs = maHoaRun(seq);
    console.log('--- PHÂN TÍCH RUN ---');
    console.log(`Tổng số run: ${runs.length}`);
    console.log(`Run dài nhất: ${Math.max(...runs.map(r => r.length))}`);
    console.log(`Run gần nhất: ${runs[runs.length-1].side} × ${runs[runs.length-1].length}`);
    console.log('');

    // kết quả dự đoán
    const kq = phanTichTongHop(seq);
    console.log('═══════════════════════════════════════');
    console.log(`  DỰ ĐOÁN PHIÊN #${phienHienTai + 1}: ${kq.duDoan}`);
    console.log(`  ĐỘ TIN CẬY: ${(kq.doTinCay * 100).toFixed(2)}%`);
    console.log(`  ĐỒNG THUẬN: ${(kq.dongThuan * 100).toFixed(1)}% (${kq.soTai}T / ${kq.soXiu}X)`);
    console.log(`  ĐIỂM: TÀI ${kq.diemTai} | XỈU ${kq.diemXiu}`);
    console.log('═══════════════════════════════════════\n');

    console.log(`--- ${kq.soTinHieu} TÍN HIỆU ---`);
    kq.signals.forEach((s, i) => {
      const dir = s.duDoan === 'TAI' ? '🔴' : '🟢';
      console.log(`  ${(i+1).toString().padStart(2)}. ${dir} ${s.ten.padEnd(20)} ${s.duDoan.padEnd(4)} ${(s.doTinCay*100).toFixed(0).padStart(3)}% | ${s.moTa}`);
    });
...
module.exports = { phanTichTongHop, layDuLieu, taoChuoi, maHoaRun };
...
<div class="wrap">
  <header>
    <div class="brand">
      <div class="logo">HUYHOANG</div>
      <div>
        <h1>Công Nghệ Vip HUYHOANG</h1>
  ...
  <footer>
    <div>Developer: <strong>HUY HOANG</strong> · Công Nghệ Vip PAK 2026</div>
    <div id="footTime">--</div>
  </footer>
</div>
...
app.listen(PORT, () => {
  console.log('[PAK] Công Nghệ Vip huyhoang 2026 — Phân loại Cầu Chi tiết');
  console.log('[PAK] Server: http://localhost:' + PORT);
  console.log('[PAK] Developer: HUY HOANG ');
});
